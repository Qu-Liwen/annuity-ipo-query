...
with tab1:
    ...
    col1.metric("年金明细记录数", len(show_df))
    col2.metric("年金计划数量", len(summary_df))
...
with tab2:
    ...
    a_df = detail_df[
        detail_df[investor_col].str.contains(org_a, case=False, na=False)
    ]

    b_df = detail_df[
        detail_df[investor_col].str.contains(org_b, case=False, na=False)
    ]
